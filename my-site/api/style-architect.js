...
You MUST respond with valid JSON only ΓÇö no markdown, no explanation text outside the JSON.
...
1. Daily Office Wear (icon: ≡ƒÅó)
2. Important Meetings (icon: ≡ƒôè)
3. Friday Dressing (icon: ≡ƒÄë)
4. Official Parties (icon: ≡ƒÑé)
5. LinkedIn Profile Picture (icon: ≡ƒô╕)

RULES:
- Recommendations MUST be specific (brand names, exact colors, exact styles) ΓÇö not generic
- Consider the client's body type, skin tone, and height/weight for flattering fits
- Consider their budget and preferred brands ΓÇö suggest within their range
- Consider their city and company culture for appropriateness
- Consider their target role ΓÇö dress for the position they aspire to
- Consider their values ΓÇö style should express these values visually
...
- Monthly Budget: Γé╣${formData.budget}
...
Generate a comprehensive, personalized style blueprint for this client across all 5 occasions. Be extremely specific ΓÇö name exact garment types, fabrics, colors (using the client's flattering palette based on their skin tone), and brands within their budget.`;
  ...
  try {
    // Free models on OpenRouter ΓÇö multiple fallbacks in case one is rate-limited
    const models = [
